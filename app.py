...
@@ -90,110 +119,170 @@ def build_fill(style: str, base: str, c2: str, extra_color: str, angle: int, gap
         return (defs, 'url(#rg)')
...
+    W = H = 148
+    highlight = lighten(cfg["primary"], 0.35)
+    shadow = darken(cfg["primary"], 0.45)
...
+    )
+    parts.append(
+        f'<ellipse cx="50" cy="48" rx="28" ry="24" fill="{highlight}" opacity="0.45" />'
+    )
+    parts.append(
+        f'<ellipse cx="101" cy="104" rx="30" ry="28" fill="{shadow}" opacity="0.35" />'
+    )
...
+        parts.append(fill_defs)
+    parts.append(
+        f'<ellipse cx="70" cy="70" rx="66" ry="66" fill="{fill_ref}" '
+        f'{outline(stroke_col, stroke_w)} />'
+    )
+    parts.append(
+        f'<ellipse cx="44" cy="40" rx="30" ry="26" fill="{highlight}" opacity="0.5" />'
+    )
+    parts.append(
+        f'<ellipse cx="98" cy="96" rx="34" ry="32" fill="{shadow}" opacity="0.32" />'
+    )
     parts.append(svg_footer())
     return "\n".join(parts)
...
+    )
+    parts.append(
+        f'<ellipse cx="25" cy="22" rx="12" ry="10" fill="{highlight}" opacity="0.55" />'
+    )
+    parts.append(
+        f'<ellipse cx="50" cy="52" rx="14" ry="12" fill="{shadow}" opacity="0.35" />'
+    )
+    parts.append(svg_footer())
+    return "\n".join(parts)
+
...
+    W = H = 38
+    highlight = lighten(cfg["primary"], 0.3)
+    shadow = darken(cfg["primary"], 0.5)
+    parts = [svg_header(W, H)]
+    if fill_defs:
+        parts.append(fill_defs)
+    parts.append(
...
+    )
+    parts.append(
+        f'<ellipse cx="12" cy="14" rx="6.5" ry="4.5" fill="{highlight}" opacity="0.55" />'
+    )
+    parts.append(
+        f'<ellipse cx="24" cy="24" rx="7" ry="4.8" fill="{shadow}" opacity="0.35" />'
+    )
     parts.append(svg_footer())
     return "\n".join(parts)
...
+    rarity: Optional[str]
     ...
     def ts_block(self, ident: str, filenames, tints) -> str:
         ...
         if self.lore:
@@ -220,51 +309,51 @@ class ExportOpts:
...
+rarity_label = st.sidebar.selectbox("Rarity", [label for label, _ in RARITY_OPTIONS], index=0)
 ...
 def part_controls(title, defaults):
     st.sidebar.markdown("---")
@@ -274,133 +363,187 @@ def part_controls(title, defaults):
...
+st.caption(
+    "All settings on the left. Preview shows a layered mock-up (backpack, body, hands, feet) plus individual sprites."
+)
...
+  <div style="position:relative;width:220px;height:220px;flex:0 0 auto;background:linear-gradient(135deg,#21435a,#3b7a8f);border-radius:16px;padding:12px;box-shadow:0 4px 18px rgba(0,0,0,0.3);">
...
+    <img src="{feet_uri}" style="position:absolute;left:72px;top:166px;width:38px;height:38px;image-rendering:optimizeQuality;" alt="Left foot" />
+    <img src="{feet_uri}" style="position:absolute;left:110px;top:166px;width:38px;height:38px;transform:scaleX(-1);image-rendering:optimizeQuality;" alt="Right foot" />
+  </div>
